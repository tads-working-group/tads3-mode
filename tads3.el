...
keywords, properties, classes, objects, and functions."
    (let ((in-property (equal (char-before (- (point) (length prefix))) ?\.))
        ...
        (maphash
            (lambda (keyword type)
                (let ((is-prop-type (or (equal type "property")
                                        (equal type "sub-object")
                                        (equal type "method"))))
                    (when (and (string-prefix-p (downcase prefix) (downcase keyword))
                                   (if in-property
                                       is-prop-type
                                       (not is-prop-type)))
                        (push (propertize keyword 'meta type) candidates))))
...
match($0, /^[ \\t]+(\\w+) = /, a) { if (!seen[a[1]]++) print a[1], \"property\" }
")
...
keywords list."
    ...
    (let* ((files (mapconcat (lambda (file) (concat "'" (expand-file-name file) "'"))
              ...
              (ident-process (start-process-shell-command
                                 "get-ident-process"
                                 ident-output
                                 (concat "awk '" tads3--identifiers-program "' " files))))
            ...
            (lambda (proc event)
                ...
                (when (and (equal (process-status proc) 'exit)
                    ...
                    (with-current-buffer (process-buffer proc)
                        (let ((keywords (-map (lambda (x) (string-split x " ")) (string-split (buffer-string) "\n"))))

                            ;; add new keywords
                            (dolist (ident keywords)
                                (puthash (car ident) (cadr ident) tads3--company-keywords))
                            (message "lines: %d" (length keywords)))))))
