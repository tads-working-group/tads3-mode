...
(defvar tads3--company-keywords
    '(
         ...
         ("static" "keyword")
         ("finally" "keyword"))
    "TADS 3 keywords, generated by a script run periodically on your project and included libraries.")
...
keywords, properties, classes, objects, and functions."
    (let ((in-property (equal (char-before (- (point) (length prefix))) ?\.)))
        (-map (lambda (item) (propertize (car item) 'meta (cadr item)))
            (-filter (lambda (item) (and (string-prefix-p prefix (car item))
                                   (if in-property
...
(defvar tads3--install-dir "~/.emacs.d/vendor/tads3-mode/")
...
keywords list."
    (let* ((files (mapconcat (lambda (file) (concat "'" (expand-file-name file) "'"))
                      (-filter
                      ...
                      " "))
              (keywords (-map (lambda (line) (string-split line " "))
                            (string-split
                                (shell-command-to-string (concat tads3--install-dir "find-identifiers.awk " files))
                                "\n"))))

        ;; add new keywords
        (dolist (ident keywords)
            (when (not (-find (lambda (x) (equal (car x) (car ident))) tads3--company-keywords))
                (setq-local tads3--company-keywords
                    (append tads3--company-keywords (list ident)))))
        ...
        (setq-local tads3--last-source-update-time
